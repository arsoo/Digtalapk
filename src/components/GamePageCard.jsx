...
export default function GamePageCard({ game, size = "normal", seoAnchor, showCta = true }) {
  ...
  if (size === "small") {
    return (
      <Link
        href={`/${game.slug}`}
        className="flex items-center gap-3 bg-bg-card border border-border-card rounded-xl p-3 hover:border-accent hover:-translate-y-0.5 transition-[border-color,transform] duration-300 group w-full"
        title={`Download ${game.t} APK – Real Money Earning App in Pakistan`}
      >
        <div className="w-12 h-12 rounded-lg overflow-hidden bg-[#080c12] relative shrink-0">
          <div
  ...
  return (
    <Link
      ...
      title={seoAnchor || `Download ${game.t} APK Pakistan`}
    >
